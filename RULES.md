...
2. **추가 파일/페이지가 필요하다고 판단되면**, 먼저 아래 형식으로 제안합니다:
   ```
   📄 추가 페이지 제안
   - 제목: [파일명]
   - 목적: [왜 필요한지]
   - 내용 요약: [무엇을 담을지]
   ```
3. **`승인: 생성`** 이라고 답하면 AI가 해당 파일을 생성/수정합니다.
4. **작업 주체**
   - 기본적으로 AI가 파일 생성/수정까지 수행합니다.
   - 필요 시 사용자에게 수동 작업을 요청할 수 있습니다.
5. **결과 안내**
   - AI는 변경된 파일 경로와 핵심 변경 사항을 요약합니다.
